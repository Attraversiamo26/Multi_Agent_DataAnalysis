...
class RAGHelper:
    ...
    def __init__(self, extract_llm):
        ...
        if system == "Windows":
            self.csv_data_directory = csv_data_dir_config.get("windows", "")
        elif system == "Darwin":
            self.csv_data_directory = csv_data_dir_config.get("macOS", "")
        else:
            self.csv_data_directory = csv_data_dir_config.get("linux", "")
    ...
    def _get_agent_data_sources_info(self) -> str:
        ...
        """
        from src.utils.scan_files import scan_directory, analyze_file, get_file_size
        ...
        try:
            ...
            for agent_name, data_sources in agent_data_sources.items():
                ...
                # Support both "csv" and "data" keys
                csv_files = data_sources.get("csv", []) or data_sources.get("data", [])
                if csv_files and self.csv_data_directory and os.path.exists(self.csv_data_directory):
                    result.append("### Available Data Files\n")
                    try:
                        ...
                        # Also try to scan Excel files manually since scan_directory only does CSV
                        all_files_info = []
                        all_files_info.extend(csv_info)
                        
                        # Add Excel files
                        path = Path(self.csv_data_directory)
                        ...
                        for excel_file in excel_files:
                            try:
                                ...
                                file_info = {
                                    ...
                                    'file_path': str(excel_file).replace('\\', '/'),
                                    'file_separator': 'Excel',
                                    'file_size': get_file_size(str(excel_file)),
                                ...
                                logger.warning(f"Error reading Excel file {excel_file}: {e}")
                        
                        # Filter files specified in configuration
                        filtered_files_info = [
                            file_info for file_info in all_files_info 
                            if file_info.get('file_name') in csv_files
                        ]
                        
                        if filtered_files_info:
                        ...
                        logger.error(f"Error scanning data directory for {agent_name}: {e}")
                        result.append(f"(Error scanning files: {e})\n")
                elif csv_files:
                    result.append("### Data Files (configured)\n")
                    for csv_file in csv_files:
                        result.append(f"- {csv_file}\n")
