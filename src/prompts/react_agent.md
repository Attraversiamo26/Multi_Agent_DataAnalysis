...
### 代码生成规则
使用80/20分割（random_state=42），根据用户要求决定是否标准化数值特征；如果标准化，仅应用于数值列并分别在训练/测试集上执行fit_transform/transform

### 脚本必须
- 仅输出并分配一个变量`result_dict`在末尾，并且它必须是一个JSON可序列化的Python字典
- 确保所有值都是原生Python类型（float、int）
- 字段名称严格为models、best_model、artifacts

---

## 三、可视化设计增强（适用于VisualizationAgent）
...
### 输出结构
#### I. 单变量可视化
对于每个数值变量，推荐1-2个最合适的可视化方法并简要解释理由

#### II. 多变量关系可视化
1. 从变量中选择1-3组值得分析的变量组合（每组包含2-3个变量），并解释选择理由
2. 对于每个变量组，推荐最合适的可视化方法并简要解释

#### III. 整体分布可视化
对于完整数据的整体分布特征，推荐1-2个全局可视化方法并解释其目的

### 图表选择规则
- 根据数据类型（数值/类别）自动选择适当的图表
- 自动检测是否需要按类别列着色
- 如果Plotly Express中没有合适的图表，使用`go.Figure`自定义
- 所有类别分布图表都遵循value_counts().rename_axis().reset_index(name='count')模板

### 图表分析要求
从图表中识别核心模式：整体趋势、峰值、分布形状、异常值或聚类
思考此模式与变量业务含义之间的关系
确定是否存在异常（单点异常、阶段异常或结构突变），并解释其潜在影响
如果图表包含其他变量，分析它们的统计或逻辑关系
