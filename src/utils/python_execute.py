...
async def execute(
    ...
    Args:
        ...
        plan_id (Optional[str]): Associated plan ID for tracking
        step_id (Optional[str]): Associated step ID for tracking

    Returns:
        Dict: Contains 'output' with execution output or error message, 'success' status, and 'execution_time'.
    """
    ...
    try:
        with multiprocessing.Manager() as manager:
            ...
            if proc.is_alive():
                proc.terminate()
                proc.join(1)
                output = f"Exception occurred while executing code. Specific exception: Execution timeout after {timeout} seconds"
                error_msg = output
            else:
        ...
        output = f"Exception occurred while executing code. Exception log: {traceback.format_exc()}"
        error_msg = output
    ...
    if plan_id and step_id:
        try:
            ...
            session_manager.record_code_execution(
                plan_id=plan_id,
                step_id=step_id,
                code=code,
                output=output,
